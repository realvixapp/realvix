...
function abrirWAConMensajes(leadId) {
  ...
  // Asegurar que los textos estén cargados
      ...
      <div style="display:flex;flex:1;overflow:hidden;min-height:0;">
        <div style="flex:1;padding:16px 18px;display:flex;flex-direction:column;gap:12px;overflow-y:auto;">
          ...
          <div>
            ...
            <div id="waTextosGrid" style="display:flex;flex-direction:column;gap:5px;max-height:200px;overflow-y:auto;">
              ${textos.length > 0
                ? textos.map(t=>`
...
function usarTextoWA(textoId, leadId) {
  ...
  const c = LEADS.consultas.find(x => x.id === leadId);
  if (!t || !c) return;
  // Reemplazar atributos con valores reales
  let msg = t.contenido || '';
  msg = msg
...
async function cargarActividadLeads() {
  try {
    ...
    // Sincronizar LEADS.consultas para que abrirWAConMensajes funcione desde actividad
    LEADS.consultas  = LACT.consultas;
