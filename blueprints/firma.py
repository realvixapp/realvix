...
def crear_propiedad():
    ...
    try:
        ...
        cur = conn.cursor()
        resp = data.get('respuesta_listing','esperando_respuesta') or 'esperando_respuesta'
        base_params = (pid, user['id'], data.get('direccion',''), data.get('localidad',''),
...
def actualizar_propiedad(pid):
    ...
    try:
        def d(v): return v if v and str(v).strip() else None
        cur = conn.cursor()
        resp = data.get('respuesta_listing','') or ''
        upd_params = (data.get('direccion',''), data.get('localidad',''), data.get('zona',''),
